...
$(function () {
    ...
    function toEdit() {
        ...
        if (rows) {
            if (rows.length > 1) {
                ...
                $('#editForm').form('load', $.curreturnItem);
                setEditable($.curreturnItem.stat!=6);//已完成状态不可编辑
                t2Query();
                $('#editPanel').dialog('open');
    ...
    function save() {
        ...
        if ($('#editForm').form('validate')) {
            ...
            if (details)
                $.curreturnItem.details = JSON.stringify(details);
            delete $.curreturnItem.type;
            $.ajax({
                url: t1Url + '/save',
                type: 'post',
                data: $.curreturnItem
                ...
                if (ret && ret.flag) {
                    ...
                    if ($.curreturnItem.id) {
                        $.get(t1Url + '/findById', {id: $.curreturnItem.id}, function (data) {
    ...
    function t2ToAdd() {
        $.t2CurreturnItem = {dtlId: -(new Date().getTime()), type: 'new'};
        $('#t2EditPanel').dialog('open');
        $('#t2EditForm').form('clear');
        $('#t2EditForm').form('load', {dtlId: $.t2CurreturnItem});
    }
    ...
    function t2ToEdit() {
        ...
        if (rows) {
            if (rows.length > 1) {
                ...
                $('#t2EditForm').form('clear');
                $.t2CurreturnItem = rows[0];
                $('#t2EditForm').form('load', $.t2CurreturnItem);
                $('#t2EditPanel').dialog('open');
            }
        } else {
            $.messager.alert('系统提示!', '请选择要编辑的行!')
    ...
    function calcTotal() {
        ...
        if (rows && rows.length) {
            ...
            $.each(rows, function (i, row) {
                ...
                returnMoney += (isNaN(itemRent) ? 0 : itemRent)
    ...
    function t2Query() {
        var url = t2Url + '/findAllById?id=' + $.curreturnItem['id'];
        $.ajax({
            url: url,
            dataType: 'json',
            type: 'get'
